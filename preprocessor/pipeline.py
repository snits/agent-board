...
def _is_automated_session(session_data: dict) -> bool:
    """Detect sessions generated by automated tools (e.g., episodic-memory summarizer).

    These are typically 2-message sessions where the first message is a
    summarization prompt. They add noise and no value to the viewer.
    """
    ...
    if not messages:
        return False
    first_content = (messages[0].get("content") or "").lower()
    return "summary" in first_content and "please write" in first_content
...
def run_preprocess(source_dir: Path, output_dir: Path, archive_dir: Path | None = None) -> None:
    ...
    for project in projects:
        ...
        for session in project["sessions"]:
            ...
            if _is_automated_session(session_data):
                continue
